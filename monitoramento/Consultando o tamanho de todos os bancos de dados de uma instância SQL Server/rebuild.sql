...
BEGIN
	...
	BEGIN TRY
			...
			INSERT INTO [db_dba]..[ManutencaoIndices]
			SELECT 	BancoDeDados,Tabela,Indece,Fragmentacao,Comando,'S',NULL,GETDATE()
				  FROM @TabelaDefrag
			WHERE Linha = @MIN 
	...
	BEGIN CATCH
		...
		INSERT INTO [db_dba]..[ManutencaoIndices]
			SELECT 	BancoDeDados,Tabela,Indece,Fragmentacao,Comando,'S',ERROR_MESSAGE(),GETDATE()
				  FROM @TabelaDefrag
			WHERE Linha = @MIN
	END CATCH
