...
FROM sys.dm_db_missing_index_group_stats AS migs
     ...
     JOIN sys.dm_db_missing_index_details AS mid ON mig.index_handle = mid.index_handle
                                                   AND database_id = DB_ID('st1714_l3')
...
FROM sys.dm_db_missing_index_group_stats AS migs
     JOIN sys.dm_db_missing_index_groups AS mig ON migs.group_handle = mig.index_group_handle
     JOIN sys.dm_db_missing_index_details AS mid ON mig.index_handle = mid.index_handle
                                                    AND database_id = DB_ID('st1714_l3');--and mid.object_id = object_id(‘tabela’)  -- se desejar ver apenas para uma      tabela específica order by Impacto DESC;
...
FROM sys.dm_db_index_usage_stats vw
     ...
     JOIN sys.dm_db_index_operational_stats(DB_ID('st1714_l3'), NULL, NULL, NULL) vwx ON vwx.object_id = tb.object_id
                                                                                              AND vwx.index_id = ix.index_id
WHERE vw.database_id = DB_ID('st1714_l3')
      AND vw.user_seeks = 0
      AND vw.user_scans = 0
...
FROM sys.dm_db_index_usage_stats vw
     ...
     JOIN sys.dm_db_index_operational_stats(DB_ID('st1714_l3'), OBJECT_ID(N'Log'), NULL, NULL) vwx ON vwx.index_id = ix.index_id
                                                                                                      AND ix.object_id = vwx.object_id
     JOIN sys.dm_db_index_physical_stats(DB_ID('st1714_l3'), OBJECT_ID(N'Log'), NULL, NULL, 'SAMPLED') vwy ON vwy.index_id = ix.index_id
