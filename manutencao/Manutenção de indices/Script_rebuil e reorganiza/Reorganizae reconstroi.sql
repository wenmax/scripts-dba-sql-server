...
SET @SQLServerVersion = (
		SELECT CAST(LEFT(CAST(SERVERPROPERTY('ProductVersion') AS VARCHAR(50)), CHARINDEX('.', CAST(SERVERPROPERTY('ProductVersion') AS VARCHAR(50))) - 1) AS INT)
		);

IF @isSysAdmin = 1
	AND @SQLServerVersion >= 9
BEGIN
	--
	...
	---------------------------------------------------------
	--if set to 1: O script apenas exibirá a reorganização do índice ou construirá os comandos sem executá-los.
	--if set to 0: O script irá reorganizar ou reconstruir os índices fragmentados.
	SET @reportOnly = 1;
	--optional: if not set (NULL), it will scann all databases
	--If name is set (i.e. 'testDB') it will just scan the given database (colocar o nome da base a ser scaneada)
	SET @databaseToCheck = 'SCOClient';
	...
	BEGIN
		...
		IF @checkResult < 1
			RAISERROR (
					'Error executing index reorganization/rebuild script: Database does not exist'
					,20
					,1
					)
			WITH LOG;
		...
		IF @checkResult < 90
			RAISERROR (
					'Error executing index reorganization/rebuild script: Only databases with SQL Server 2005 or later compatibility level are supported'
					,20
					,1
					)
			WITH LOG;
