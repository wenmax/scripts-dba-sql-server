...
JOIN sys.dm_db_index_operational_stats(db_id('Siga_CAI_Public'), OBJECT_ID(N'Log'), NULL, NULL) vwx ON vwx.index_id = ix.index_id
	AND ix.object_id = vwx.object_id
JOIN sys.dm_db_index_physical_stats(db_id('Siga_CAI_Public'), OBJECT_ID(N'Log'), NULL, NULL, 'SAMPLED') vwy ON vwy.index_id = ix.index_id
...
WHERE vw.database_id = db_id('Siga_CAI_Public')
